...
# print("API KEY:", settings.ANTHROPIC_API_KEY)
...
def create_health_checkin(checkin: HealthCheckInCreate, user_id: int = Query(...), db: Session = Depends(get_db)):
...
def create_mood_entry(mood: MoodEntryCreate, user_id: int = Query(...), db: Session = Depends(get_db)):
...
def add_medication(med: MedicationCreate, user_id: int = Query(...), db: Session = Depends(get_db)):
...
def daily_checkin(user_id: int = Query(...), db: Session = Depends(get_db)):
